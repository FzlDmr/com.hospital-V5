...
public class PatientsMethods implements WrongInput {
    ...
    public void findPatientFromDisease(){

        new ListDiseases().printDiseaseList();
        ...
        try {
            ...
            CriteriaQuery<Patient> patient = cb.createQuery(Patient.class);
            Root<Patient> root = patient.from(Patient.class);
            patient.select(root)
                    .where(cb.equal(root.get("disease"), "ECZEMA"));

              List<Patient> resultList1 = session.createQuery(patient).getResultList();
